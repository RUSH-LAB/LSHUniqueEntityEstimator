...
for ((i=6;i<=20;i+=4)) ; 
	do for ((j=1;j<=3; j++));
 		do ./output config_cd.txt 1 $i; python pipeline.py --flag 0 --id $i --trainsize 0.5 --input Cd_pair.csv --goldstan replicate/cd.csv --delimiter ';' --output log-cd ; 
	done
done
...
# for ((i=25;i<=40;i+=5)) ; 
# 	do for ((j=1;j<=10; j++));
#  		do ./output config_voter.txt 4 $i; python pipeline.py --flag 0 --id $i --trainsize 0.1 --input Voter_pair.csv --goldstan replicate/voter_processed.csv --delimiter ',' --c 0.0001 --output log-voter ; 
# 	done
# done
